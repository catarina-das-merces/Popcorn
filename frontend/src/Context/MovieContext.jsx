...
export const MovieProvider = ({ children }) => {
	...
	const fetchMovies = async () => {
		try {
			...
			const response = await fetch(
				"https://popcorn-backend-jc0e.onrender.com/api/movies",
				{
					headers: {
	...
	const handleUpdateMovie = async (event) => {
		...
		try {
			...
			const response = await fetch(
				`https://popcorn-backend-jc0e.onrender.com/api/movies/${selectedMovie._id}`,
				{
					method: "PUT",
					headers: {
						Authorization: `Bearer ${accessToken}`,
	...
	const handleDeleteMovie = async (movieId) => {
		try {
			...
			const movie = movies.find((m) => m._id === movieId);
			const response = await fetch(
				`https://popcorn-backend-jc0e.onrender.com/api/movies/${movieId}`,
				{
					method: "DELETE",
					headers: {
						Authorization: `Bearer ${accessToken}`,
	...
	const rateMovie = async (movieId, rating) => {
		try {
			...
			const response = await fetch(
				`https://popcorn-backend-jc0e.onrender.com/api/movies/${movieId}/rate`,
				{
					method: "POST",
					headers: {
